...
@Component({
  selector: 'app-sale-by-forms',
  templateUrl: './sale-by-forms.component.html',
  styleUrls: ['./sale-by-forms.component.scss']
})
export class SaleByFormsComponent implements OnInit {

  salesByFormColumns: string[] = ["form", "qty", "ppv"];

  salesByFormsDataSource = new MatTableDataSource<any>();

  startDate = moment(new Date()).format(" D/MM/YYYY");
  endDate = moment(new Date()).format(" D/MM/YYYY");
  private isLoading: boolean;

  @ViewChild("saleFormsPaginator") set productPaginator(pager: MatPaginator) {
    if (pager) this.salesByFormsDataSource.paginator = pager;
  }
  ...
  async ngOnInit(): Promise<void> {
    this.isLoading = true;
    await this.salesByForms({ startDate: new Date(), endDate: new Date() });
    this.isLoading = false;
  }

  applyFilterOnSaleByProduct(event: Event): void {
    this.applyFilter(event, this.salesByFormsDataSource);
  }
  ...
  async openQueryPeriodDialog(): Promise<void> {
    ...
    this.isLoading = true;
    await this.salesByForms(result);
    this.isLoading = false;
  }
  private async salesByForms(dateRange: any) {
    const response = await this.orderService
      .loadSalesByForms(dateRange)
      .toPromise();

    if (response?.errorCode === 2) {
      this.dispatchError(response);
    } else {
      this.salesByFormsDataSource.data = response?.results;
    }
  }
